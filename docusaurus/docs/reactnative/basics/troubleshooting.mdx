...
- While you are on `react-native-gesture-handler` version >=2.0.0, do not forget to wrap your component tree(probably above `OverlayProvider`) with `<GestureHandlerRootView>` or `gestureHandlerRootHOC`. Not doing so, can cause unusual behaviour with the `MessageOverlay` and `Imagegallery` gestures.
